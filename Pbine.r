...
integral_method = function(pm, r = NULL, method = "Int", w = NULL, max_exp = 17, rel_error = 1e-5){
	if(is.null(r)){
		...
		if(is.na(r)){stop("please enter the correlation r")}
	}
	test_f = Vectorize(test_f)
	
	if(r==1) stop("correlation r must be less than 1")
	if(length(w)!=0){
		if(length(w)!=1) stop("The weight w is a vector with length quals 1. (The test statistic is p1^w*p2^(1-w).)")
		w = w
		method = "int_weighted"
	}
	if(method == "Int"){
...
Ibind_n_dim = function(pm, sigma, w = NULL){
	C = t(chol(sigma))
	C_inv = solve(C)
	n_combine = dim(C)[1]
	for(i in 1:n_combine){
		eval(parse(text = (paste("p", i, "=pm[ ,", i, "]", sep = ""))))
		if(length(w)!=0){
			eval(parse(text = (paste("w", i, "=w[", i, "]", sep = ""))))
		}else{
			eval(parse(text = (paste("w", i, "=1", sep = ""))))
		}
	}
	der_erf = function(x) {2/(pi^0.5)*exp(-x^2)}
	der_norm = function(x) {1/(2*2^0.5)*der_erf(x/(2^0.5))}
	der_qnorm = function(x) {1/(der_norm(qnorm(x)))}
	input_x_text = paste(paste("x", 1:n_combine, collapse=",", sep=""), paste("w", 1:n_combine, collapse=",", sep=""), sep=",")
	for(i in 2:n_combine){
		eval(parse(text = (paste("der_var_", i, "=function(", input_x_text, "){",
		"return(-der_norm(", paste(C_inv[i,], paste("qnorm(1-exp(", paste("x", 1:n_combine, "/w", 1:n_combine, sep=""), "))", sep=""), sep="*", collapse="+"), "))}", sep=""))))
		eval(parse(text = (paste("der_var_", i, "=Vectorize(der_var_", i, ")", sep=""))))
	}
	
	eval(parse(text = (paste("test_f_nd=function(", input_x_text, "){",
		"if(", paste("is.na(qnorm(1-exp(", paste("x", 1:n_combine, "/w", 1:n_combine, sep=""), ")))", sep="", collapse=" | "), "){return(0)};",
		"return(abs(", paste("(1/", diag(C), ")", collapse="*", sep=""), "*", paste("der_var_", 2:n_combine, "(", input_x_text, ")", sep="", collapse="*"), "*",
			paste("der_qnorm(1-exp(", paste("x", 2:n_combine, "/w", 2:n_combine, sep=""), "))", sep="", collapse="*"), "*",
			paste("exp(", paste("x", 1:n_combine, "/w", 1:n_combine, sep=""), ")", "/w", 1:n_combine, sep="", collapse="*"), "))}", sep=""))))
	...
	for(i in 1:n_combine){
		if(!i==n_combine){
			...
			paste("w", 1:n_combine, "*", "log(p", 1:n_combine, "[xx])", sep="", collapse="+"), "-", paste("x", 1:(n_combine-i), sep="", collapse="-"), ",0)$value})", sep="")
		}else{
			tailint_text = paste(tailint_text, "}, ",
			paste("w", 1:n_combine, "*", "log(p", 1:n_combine, "[xx])", sep="", collapse="+"), ",0)$value)", sep="")
		}
	}
...
Pbine=function(pm, sigma = NULL, method = "Int", w = NULL){
	...
	if(dim(pm)[2] > 2) {return(Ibind_n_dim(pm = pm, sigma = sigma, w = w))}
}
...
pn_w = Pbine(pm, sigma = COV, method = "Int", w = c(1/2, 1/3, 1/6)) #Weighted Fisher.
print(c(p1, p2, pn))
print(c(p1, p2, pn_w))
